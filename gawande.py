...
def main():
    loader = FileSystemLoader(
            os.path.join(os.path.dirname(
                os.path.abspath(__file__)), 'checklists'))
    env = Environment(loader=loader)
    parser = argparse.ArgumentParser(
        description="Quick access to checklists.")
    ...
    if args.checklist:
        ...
        checklist_dir = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), 'checklists')
        print("""\nAvailable checklists:\n---------------------""")
        for checklist in os.listdir(checklist_dir):
            print("\t " + checklist.replace(".md", ""))
        print("\n\nTo see checklist, run gawande.py\
